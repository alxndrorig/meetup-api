...
export const generateJWT = async (id, role) => {
  try {
    ...
    const userToken = await dbConnection.oneOrNone(
      'SELECT * FROM usertokens WHERE "userId" = $1',
      [id]
    );
    if (userToken) {
      await dbConnection.result('DELETE FROM usertokens WHERE id = $1', [
        userToken.id,
      ]);
    }

    const { userId, token, createdAt } = new UserToken({
      userId: id,
      token: refreshToken,
      createdAt: new Date(),
    });
    ...
    dbConnection.one(
      'INSERT INTO usertokens("userId", token, "createdAt") VALUES($1, $2, $3) RETURNING *',
