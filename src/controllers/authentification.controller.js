...
const login = async (req, res, next) => {
    passport.authenticate('local', {failureRedirect: '/login'}, async (error, user) => {
        if (error) {
...
const getAccessToken = async (req, res, next) => {
    try {
        ...
        const { accessToken } = await generateJWT(tokenDetails.id, tokenDetails.role);
        res.status(200).json({message: "Access token created succesfully", token: accessToken});
    } catch (error) {
...
export { registration, login, getAccessToken, loggingOut };